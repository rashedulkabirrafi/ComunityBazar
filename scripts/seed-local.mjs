...
export async function seed() {
  ...
  try {
    const db = client.db('CampusBazar-local');
    const photoURL = '/favicon.svg';
    for (const [email, name, role] of [
      ['student@campusbazar.test', 'Local Student', 'general user'],
      ['admin@campusbazar.test', 'Local Admin', 'admin'],
    ]) {
      const call = async (method, body) => {
        const response = await fetch(`http://127.0.0.1:9099/identitytoolkit.googleapis.com/v1/accounts:${method}?key=demo-campus-bazar-key`, {
    ...
    for (const [key, name, category, price] of [
      ...
      await db.collection('listings').updateOne({ localSeedKey: key }, { $setOnInsert: {
        ...
        image: photoURL, email: 'student@campusbazar.test', date: '2026-09-05', createdAt: new Date(),
      } }, { upsert: true });
    }
