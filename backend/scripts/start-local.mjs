...
async function stop(code = 0) {
  ...
  try {
    if (Number(readFileSync(".local-data/stack.pid", "utf8")) === process.pid)
      unlinkSync(".local-data/stack.pid");
  } catch {}
  process.exit(code);
}
...
function start(name, args, cwd = root) {
  const log = createWriteStream(`${root}.local-data/logs/${name}.log`, {
    flags: "a",
  });
  ...
  child.on("exit", (code) => {
    if (!stopping) {
      console.error(
        `${name} exited (${code}). See .local-data/logs/${name}.log`,
      );
      stop(1);
    }
  });
...
async function ready(url) {
  ...
  throw new Error(
    `Service did not become ready: ${url}. Check .local-data/logs.`,
  );
}
async function freePort(port) {
  await new Promise((resolve, reject) => {
    ...
    server.once("error", () =>
      reject(
        new Error(
          `Port ${port} is already in use. Stop the other local stack before starting this one.`,
        ),
      ),
    );
...
try {
  for (const port of [27017, 9099, 4400, 4500, 3000, 5173])
    await freePort(port);
    ...
    instanceOpts: [
      {
        ip: "127.0.0.1",
        port: 27017,
        dbPath: `${root}.local-data/mongodb`,
  ...
  console.log(
    "Local-only test accounts. Press Ctrl+C to stop and save authentication data.",
  );
} catch (error) {
  console.error(error);
  await stop(1);
